...
export default function Inventory() {
  ...
  const [showUseProductModal, setShowUseProductModal] = useState(false);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={() => setShowUseProductModal(true)} className="bg-gradient-primary">
      ...
      <Card className="bg-gradient-card border-border">
        <CardHeader>
          <CardTitle>Use Product History</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="overflow-x-auto">
            <Table>
              ...
              <TableBody>
                {(() => {
                  const useLogs = logs.filter((log: any) => log.type === 'deduct');
                  return useLogs.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={7} className="text-center py-8 text-muted-foreground">
                        No use product history
                      </TableCell>
                    </TableRow>
                  ) : (
                    useLogs.slice(0, 50).map((log: any) => {
                      const itemName = accessoriesData.find((item: any) => item.id === log.item_id)?.item_name || log.inventory_items?.item_name || '-';
                      return (
                        <TableRow key={log.id}>
                          <TableCell>{formatDate(log.created_at)}</TableCell>
                          <TableCell>{itemName}</TableCell>
                          <TableCell>
                            <Badge className="bg-warning">Used</Badge>
                          </TableCell>
                          ...
                          <TableCell>
                            <div className="flex gap-2">
                              <Button variant="outline" size="sm" onClick={() => { setSelectedLog(log); setShowEditLogModal(true); }}>
                                <Edit className="h-4 w-4" />
                              </Button>
                              <Button variant="outline" size="sm" onClick={() => handleDeleteLog(log.id)} className="border-destructive text-destructive">
      ...
      <UseProductModal open={showUseProductModal} onClose={() => setShowUseProductModal(false)} accessoriesData={accessoriesData} machines={machines || []} />
      <EditLogModal open={showEditLogModal} onClose={() => setShowEditLogModal(false)} log={selectedLog} accessoriesData={accessoriesData} machines={machines || []} />
    </div>
  );
}
...
function EditLogModal({ open, onClose, log, accessoriesData, machines }: any) {
  ...
  const itemName = accessoriesData.find((item: any) => item.id === log.item_id)?.item_name || '-';
  
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Edit Use Product Log</DialogTitle>
        </DialogHeader>
        <div className="space-y-4">
          <div>
            <Label>Item Name</Label>
...
function UseProductModal({ open, onClose, accessoriesData, machines }: any) {
  const { user } = useAuth();
  const queryClient = useQueryClient();
  ...
  const selectedItem = accessoriesData.find((item: any) => item.id === selectedItemId);
  ...
  const handleSubmit = async () => {
    ...
    if (quantity > availableQuantity) {
      toast.error(`Quantity exceeds available stock (${availableQuantity})`);
      return;
    }
    
    try {
      const newQuantity = availableQuantity - quantity;
      const newTotalAmount = newQuantity * (selectedItem.item_price || 0);
      
      await db.from('machine_expenses')
        .update({ 
      ...
      await db.from('inventory_logs').insert({
        ...
        remarks: remarks || `Used product${machineId !== 'none' ? ` for machine` : ''}`,
      ...
      toast.error(`Failed to use product: ${error.message || 'Unknown error'}`);
    }
  };
    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent>
        ...
        <div className="space-y-4">
          ...
          {selectedItem && (
            <div>
              <Label>Available Quantity: {availableQuantity}</Label>
            </div>
          )}
          <div>
            <Label>Quantity *</Label>
